...
- **⚠️⚠️ ATTENZIONE GRAVE — cartella `Galleria PL/`**: i file in `Galleria PL/img/` (`p-1.jpg`…`p-8.jpg`) **NON sono foto di workshop: sono SCANSIONI DI PASSAPORTI** (Federazione Russa e Lituania) con dati personali completi di persone reali (nomi, date di nascita, n. documento, fototessere). Probabilmente raccolte per organizzare un viaggio/study visit. **NON DEVONO MAI essere pubblicate online** (privacy/GDPR). La cartella `Galleria PL/` è stata **aggiunta a `.gitignore`** (esclusa dal repo); la pagina `Galleria PL.html` e il link nella Home (voce Poland→Foto) sono stati **rimossi/non attivati**. La voce «Foto» PL resta su `Partner` con `--soon`. Se in futuro arrivano VERE foto: svuotare `img/` dai passaporti, mettere le foto reali, ricreare l'HTML (clone di `Galleria DE`), e SOLO ALLORA togliere `Galleria PL/` dal `.gitignore` e attivare il link.
